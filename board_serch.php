...
echo "<link rel=stylesheet type=text/css href=../include/my.css>";
    ...
    $word=$_POST[txt_serch];
    
    $qurey="select * from board where subject like '%$word%' order by num desc";
    $relist = mysql_query($qurey) or die (mysql_error());

    echo "<input type='button' value='이전으로' class=main onclick=location.href='board_list.php'>";
    echo "<center><table border=1 width=600 height=350> ";
    ...
    while($relist){
        ...
        if(!$list) break;
            ...
            echo "<td> $list[num]</td>
                      <td> $list[id]</td>
                      <td width=250><a href='board_view.php?num=$list[num]'> $list[subject]</a></td> ";
        ...
        if($block > 1){
            $prev=$first;
            echo "<a href='board_serch.php?page=1'>[처음]</a>&nbsp; ";
            echo "<a href='board_serch.php?page=$prev'>[◀]</a>";
        }
        
        if($page > 1) {
            $go_page=$page-1;
            echo " <a href='board_serch.php?page=$go_page'>[이전]</a>&nbsp; ";
        }
        ...
        for($page_link=$first+1; $page_link<=$last; $page_link++){
            if($page_link==$page)
                echo "<font color=green><b>$page_link</b></font>&nbsp; ";
                    ...
                    echo "<a href='board_serch.php?page=$page_link'>[$page_link]</a>&nbsp; ";
        }
        
        if($page < $total_page){
            $go_page=$page+1;
            echo "<a href='board_serch.php?page=$go_page'>[다음]</a>&nbsp;";
        }
        
        if($block < $total_block){
            $next=$last;
            echo"<a href='board_serch.php?page=$next'>[▶]</a> &nbsp;";
            echo "<a href='board_serch.php?page=$total_page'>[마지막]</a>";
        }
